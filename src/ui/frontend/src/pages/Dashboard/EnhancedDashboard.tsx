...
const EnhancedDashboard: React.FC = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      setProcessingQueue(queueRes.data.jobs || []);
  ...
  const handleMetadataSearch = async () => {
    try {
      ...
      formData.append('include_fulltext', 'true');
      ...
      setProcessingQueue(prev => [newJob, ...prev]);
      
      // Navigate to metadata results
      navigate('/knowledge-base', { state: { searchResults: response.data.results } });
    } catch (err) {
  ...
  const handleDocumentUpload = async (files: FileList) => {
    try {
      ...
      response.data.jobs.forEach((job: ProcessingJob) => {
        setProcessingQueue(prev => [job, ...prev]);
      });
  ...
  const handleValidateExtraction = async (extractionId: string) => {
    try {
      ...
      navigate(`/validation/${extractionId}`);
    } catch (err) {
        ...
        <Grid item xs={12} sm={6} md={3}>
          <Card>
            <CardContent>
              ...
              <Typography variant="h5">
                {systemStatus?.database_size ? `${(systemStatus.database_size / 1024 / 1024).toFixed(1)} MB` : '0 MB'}
              </Typography>
              <Typography variant="body2" color="textSecondary">
                records stored
              </Typography>
